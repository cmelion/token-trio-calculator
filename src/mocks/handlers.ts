...
const shouldUseMocks = () => {
    // Check for component test environments (vitest)
    const isComponentTest =
        import.meta.env.TEST === 'true' ||
        import.meta.env.VITEST === 'true' ||
        import.meta.env.NODE_ENV === 'test';

    // Check for Storybook environments (used by Playwright BDD tests)
    const isStorybookEnvironment =
        import.meta.env.STORYBOOK === 'true' ||
        import.meta.env.VITE_STORYBOOK === 'true';

    // Enable mocks for both component tests and Storybook/Playwright tests
    return isComponentTest || isStorybookEnvironment;
}
