...
const getAppContext = async (page: Page): Promise<AppContext> => {
  ...
  if (!contextData) {
    throw new Error(
      "App test context not found - ensure you run the previous steps",
    )
  }
...
When(
  "I click the {string} quick select button",
  async ({ page }: World, tokenSymbol: string) => {
    // Find and click the quick select button for the specified token
    const quickSelectButton = page.getByRole("button", {
      name: new RegExp(`Quick select ${tokenSymbol}`, "i"),
    })
...
When(
  "I click on a token that is already selected as target",
  async ({ page }: World) => {
    // First identify what token is currently the target token
    const targetCard = page.locator("div", {
      has: page.getByText("You receive", { exact: true }),
    })
    const tokenSymbolElement = targetCard
      .locator("button")
    ...
    const quickSelectButton = page.getByRole("button", {
      name: new RegExp(`Quick select ${tokenSymbol.trim()}`, "i"),
    })
    await quickSelectButton.click()
  },
)
...
When(
  /^I click the token select button on the (source|target) card$/,
  async ({ page }: World, cardType: string) => {
    // Find the exact token symbol in the card we're targeting
    let tokenSymbol
    if (cardType === "source") {
      // Get the specific token symbol from the source card
      const sourceTokenBtn = page
        .getByRole("button", { name: /Change [A-Z]+ token/ })
    ...
    const buttonSelector = page.getByRole("button", {
      name: `Change ${tokenSymbol} token`,
    })
...
When(
  "I switch the source card to token input mode",
  async ({ page }: World) => {
    // Find the source card by its header text
    const sourceCard = page.locator("div", {
      has: page.getByText("You pay", { exact: true }),
    })

    // Find the token symbol to make more specific selector
    const tokenSymbol = await sourceCard
      .locator("button")
      .filter({ hasText: /^[A-Z]{2,5}$/ })
      .first()
      .textContent()
    ...
    const newToggleButton = sourceCard.getByRole("button", {
      name: /Switch to USD input mode/i,
    })
...
Then(
  "the source input field should display the equivalent token amount",
  async ({ page }: World) => {
    ...
    const amountField = sourceCard.getByRole("textbox", {
    ...
    const usdModeToggle = sourceCard.getByRole("button", {
      name: "Switch to USD input mode",
    })
...
Then(
  "the source secondary value should show the USD equivalent",
  async ({ page }: World) => {
    // Find the source card
    const sourceCard = page.locator("div", {
      has: page.getByText("You pay", { exact: true }),
...
Then("the target amount should be updated", async ({ page }: World) => {
  ...
  const amountField = targetCard.getByRole("textbox", {
    name: "Target USD amount",
  })
...
Then(
  "I should see the exchange rate between tokens",
  async ({ page }: World) => {
    // Find the exchange rate region using its ARIA role and label
    const exchangeRateRegion = page.getByRole("region", {
...
Then(
  "I should see the exchange rate between {string} and {string}",
  async ({ page }: World, sourceSymbol: string, targetSymbol: string) => {
...
Then(
  "the exchange rate should reflect the correct conversion",
  async ({ page }: World) => {
    ...
    const sourceRateElement = sourceCard.locator(
      'span[aria-label^="Source token exchange rate"]',
    )
    const targetRateElement = targetCard.locator(
      'span[aria-label^="Target token exchange rate"]',
    )
    ...
    const sourceRateText =
      (await sourceRateElement.getAttribute("aria-label")) || ""
    const targetRateText =
      (await targetRateElement.getAttribute("aria-label")) || ""
    ...
    const sourcePriceMatch = sourceRateText.match(
      /approximately ([\d.]+) US dollars/,
    )
    const targetPriceMatch = targetRateText.match(
      /approximately ([\d.]+) US dollars/,
    )
    ...
    if (
      ...
      throw new Error(
        "Could not extract all required exchange rate information",
      )
    }
    ...
    const exchangeRateText = page.getByText(
      new RegExp(`1\\s+${sourceToken}\\s*≈\\s*[\\d.]+\\s+${targetToken}`),
    )
    ...
    if (rateMatch && rateMatch[1]) {
      ...
      expect(
        displayedRate,
        `Exchange rate between ${sourceToken} and ${targetToken} is incorrect`,
...
Then(
  "the exchange rate should show the price in USD",
  async ({ page }: World) => {
    // Find the source card
    const sourceCard = page.locator("div", {
      has: page.getByText("You pay", { exact: true }),
    ...
    const rateElement = sourceCard.locator(
      'span[aria-label^="Source token exchange rate"]',
    )
...
Given(
  ...
  async ({ page }: World, tokenSymbol: string) => {
    ...
    const tokenButton = sourceCard.getByRole("button", {
      name: new RegExp(`Change ${tokenSymbol} token`),
    })
...
Given(
  ...
  async ({ page }: World, tokenSymbol: string) => {
    ...
    // Use a more specific selector that uniquely identifies the token button
    const tokenButton = targetCard.getByRole("button", {
      name: new RegExp(`Change ${tokenSymbol} token`),
    })
    await expect(tokenButton).toBeVisible()
  },
...
Then(
  /^the (source|target) token should be "([^"]*)"$/,
  async ({ page }: World, cardType: string, tokenSymbol: string) => {
    ...
    const card = page.locator("div", {
      has: page.getByText(headerText, { exact: true }),
    })

    // Find the token button specifically using a more precise selector
    const tokenButton = card.getByRole("button", {
      name: new RegExp(`Change ${tokenSymbol} token`),
    })
    await expect(tokenButton).toBeVisible()
  },
...
Then(
  "the source and target tokens should be swapped",
  async ({ page }: World) => {
    ...
    // Get token symbols
    const sourceTokenSymbol = await sourceCard
      .locator("button")
      .filter({ hasText: /^[A-Z]{2,5}$/ })
      .first()
      .textContent()
    const targetTokenSymbol = await targetCard
      .locator("button")
      .filter({ hasText: /^[A-Z]{2,5}$/ })
      .first()
      .textContent()
    ...
    const exchangeRateContainer = page
...
When(
  ...
  async ({ page }: World, buttonText: string) => {
...
Then("I should see the wallet connection dialog", async ({ page }: World) => {
  ...
  const walletProviders = page.getByRole("listbox", {
    name: "Available wallet providers",
  })
  await expect(walletProviders).toBeVisible()
})

When(
  "I select the {string} wallet provider",
  async ({ page }: World, providerName: string) => {
    // Find and click the wallet provider button
    const walletButton = page.getByRole("option", {
    ...
    const toast = page
      ...
      .filter({ has: page.getByText("Wallet Connected") })
...
Then(
  "I should see the wallet name {string} in the header",
  async ({ page }: World, walletName: string) => {
...
Then(
  "I should see the token balances displayed on the token cards",
  async ({ page }: World) => {
...
Then(
  "I should see a balance warning if the amount exceeds my balance",
  async ({ page }: World) => {
    // Look for the toast container and check its child elements
    const toast = page.locator('li[role="status"]').filter({
      has: page.getByText(
        /Your .* balance .* is less than the amount you're trying to spend/,
      ),
    })
